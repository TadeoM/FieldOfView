...
namespace ShadowsIn2D.Visibility
{
    ...
    public class VisibilityComputer
    {
        ...
        public VisibilityComputer(Vector2 origin, float radius, float fov)
        {
        ...
        {
            float makeRectangle = 16 / 9f;

            float angleIncrease = 30;
            float segments = fov / 30;
            for (int i = 0; i < segments; i++)
            {
                AddSegment(new Vector2(origin.x - radius * makeRectangle, origin.y - radius),
                    new Vector2(origin.x + radius* makeRectangle, origin.y - radius));
            }
            //Top
            AddSegment(new Vector2(origin.x - radius * makeRectangle, origin.y - radius),
                            new Vector2(origin.x + radius* makeRectangle, origin.y - radius));
